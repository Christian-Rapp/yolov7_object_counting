...
def perform_seq_nms(numpy_boxes, numpy_scores, numpy_labels):
    ...
    # The requirement for 2 boxes to be linked between frames
    # linkage_threshold = 0.3

    # The minimum score average in a sequence for it to be counted as a sequence
    # Allows for lower confidence during NMS to make sequences more complete
    # and then filter out junk sequences
    ...
    # print(numpy_scores)
    # print(numpy_labels)
    # seq_nms(boxes=numpy_boxes, scores=numpy_scores, labels=numpy_labels, linkage_threshold=linkage, nms_threshold=nms, score_metric=score_metric)
    # numpy_boxes, numpy_scores, numpy_labels, num_objects = build_box_sequences(numpy_boxes, numpy_scores, numpy_labels, window_size=7, min_frame_sequence=3)   
    # print("\nBoxes after")
    # print(numpy_boxes)
...
def annotate_video(source, imgsz, stride, boxes, scores, labels, names, device, save_path, ext="-seq-nms.", total_obj_count=None, frame_obj_count=None):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if(total_obj_count and frame_obj_count):
            obj_text_1 = "Cows in Frame:" + str(frame_obj_count[i])
            obj_text_2 = "Total cows:" +  str(total_obj_count[i])
...
def build_box_sequences(boxes, scores, labels, linkage_threshold=0.3, window_size=5, min_frame_sequence=2, seq_confidence_threshold=0.55):
    ...
    '''
    # overlap_matrix_np = generate_overlap_matrix(boxes)
    ...
    # best_edge_matrix = generate_best_edge_matrix(boxes, should_print=True, window_size=window_size, linkage_threshold=0.4)
    
    # TODO use the overlap matrix to generatr weighted directed graph
    # Selected highest scored node
    # Do greedy path finding for next step > linkage threshold
    #   Remove path nodes from graph
    # Repeat until no possible sequences remain
    # Profit
    ...
    if min_print:
        ...
        for sq in sequences:
            # print("\t", sq, min(sq, key=lambda x:x[0]))
...
# def merge_sequences_over_threshold(sequences, threshold=0.7):



def remove_false_positive_sequences(sequences, scores, short_length_threshold=2, high_length_threshold=7, seq_confidence_threshold=0.5):
    new_sequences = []
    count = 0
    for seq in sequences:
        ...
        # Next keep sequences above short length threshold with a high max score
        # if len(seq) > short_length_threshold and max_score > seq_confidence_threshold+0.15:
        #     new_sequences.append(seq)
        #     continue
        ...
        # # Next keep really long sequences even with lower scores
        # if len(seq) > high_length_threshold:
        #     new_sequences.append(seq)
        #     continue
...
def find_sequences_graph_from_overlap(overlap_matrix, num_frames, num_boxes, scores,sequence_length_threshold=2, window_size=5, linkage_threshold=0.5):
    ...
    # Repeat until there are no more possible graphs

    # F = networkx.from_numpy_matrix(best_edge_matrix)
    scores_copy = np.copy(scores)
    print(scores)
    print(G)
    current_frame = 0
    # print("Current Frame looking at", current_frame)
    # print("Scores for frame", scores_copy[current_frame])
    sequences = []
    while(1):
...
def find_next_best_sequence(G, start_frame, start_box, scores, num_frames, window_size=5, linkage_threshold=0.5):    
    ...
    while 1:
        ...
        if len(frames_at_looking_frame) > 0:
            ...
            # print("Chose edge", choosen_edge, "to add to sequence", type(choosen_edge))
            # print("\tchose frame", choosen_edge[0])
            # print("\tchose box", choosen_edge[1], "type", type(choosen_edge[1]), )
            current_frame+=1
            current_node = G[choosen_edge]
            current_sequence.append(choosen_edge)
            looking_at_frame+= 1
            # print("\t\tnew current node", choosen_edge, "with next nodes", current_node)
...
def generate_weighted_directed_graph(overlap_matrix, num_frames, num_boxes)->networkx.Graph:
    ...
    for fdx, frame in enumerate(overlap_matrix):
        ...
        # for bdx, overlapped_frames in enumerate(frame): 
        #     print("\tBox #", bdx)
        #     # print("\t\toverlaps with", overlapped_boxes)
        #     for fjx, frame_overlap_with_bdx in enumerate(overlapped_frames): # Overlap between Frame FDX, Box BDX and Frame FJX
        #         # Don't allow connections between items in the same frame.
                
        #         if fjx == fdx: 
        #                 continue
        #         print("\t\tOverlap with Frame #", fjx)
        #         for bjx, bjx_bdx_ovlp in enumerate(frame_overlap_with_bdx): # Overlap between Frame FDX, Box BDX and Frame FJX, Box BJX
        #             # print("\t\t\tIn Frame #", fjx, "with overlap", bjx_bdx_ovlp)
        #             print("\t\t\twith box:", bjx, bjx_bdx_ovlp)
        #             if bjx_bdx_ovlp > 0:
        #                 G.add_edge((fdx, bdx), (fjx, bjx), weight=bjx_bdx_ovlp)


            # for bjx, bjx_overlaps in enumerate(overlapped_boxes): # here
            #     print("\t\tOverlaps with box #", bjx)
            #     for fjx, ovlp in enumerate(bjx_overlaps): # No wait switch these
            #         # Don't allow connections between items in the same frame.
            #         if fjx == fdx:
            #             continue
            #         print("\t\t\tIn Frame #", fjx, "with overlap", ovlp)
            #         if ovlp > 0:
            #             G.add_edge((fdx, bdx), (fjx, bjx), weight=ovlp)
...
def generate_overlap_matrix(boxes, scores=[], labels=[], linkage_threshold=0.3, window_size=5):
    ...
    for f_i in range(boxes.shape[0]):
        ...
        for f_j in range(window_start, window_end+1):
            ...
            # print("Overlap between frame #", f_i, "and ", f_j)
            frame_ij_overlap = calculate_overlap_between_two_frames(boxes_f_i, boxes_f_j, linkage_threshold)
            # print("\t\there:\t",  frame_ij_overlap)
            frame_i_all_overlaps[f_j] = frame_ij_overlap 
        # print("Overlap for Frame ", f_i)
        # print("\t", frame_i_all_overlaps)
...
def calculate_overlap_between_two_frames(boxes_f, boxes_f1, linkage_threshold):
    ...
    for i, box in enumerate(boxes_f):
        ...
        # adjacency_matrix[i] = overlaps
        # for bjx, ovlp in enumerate(overlaps):
            # print("\t\tand box #", bjx, ":", ovlp)

    # print("adjacenty matrix", adjacency_matrix)
...
if __name__ == '__main__':
    # boxes = np.load("test_boxes_full.npy")
    # scores = np.load("test_scores_full.npy")
    # labels = np.load("test_labels_full.npy")
    ...
    # Rock is
    # F1 B2 -> F2 B2 -> F3 B2
    # boxes = np.load("test_boxes.npy")
    # scores = np.load("test_scores.npy")
    # labels = np.load("test_labels.npy")

    # boxes = np.load("test_boxes_c10.npy")
    # scores = np.load("test_scores_c10.npy")
    # labels = np.load("test_labels_c10.npy")
    ...
    # boxes_modified = boxes
    # boxes_modified[]

    # perform_seq_nms(boxes, scores, labels)
    
    # The requirement for 2 boxes to be linked between frames
    linkage_threshold = 0.3

    # The minimum score in a sequence for it to be counted as a sequence
    # Allows for lower confidence during NMS to make sequences more complete
    # and then filter out junk sequences
    sequence_confidence_threshold = 0.55




    build_box_sequences(boxes, scores, labels, linkage_threshold=linkage_threshold, seq_confidence_threshold=sequence_confidence_threshold)
